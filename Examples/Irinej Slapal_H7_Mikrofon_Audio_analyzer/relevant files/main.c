...
#include "../../Drivers/CMSIS_DSP/Include/arm_math.h"
...
#define FFT_LENGTH AUDIO_BUFF_SIZE/2
...
float32_t output_fft[FFT_LENGTH/2];
...
int main(void)
{
  /* USER CODE BEGIN 1 */
	  ...
	  //UTIL_LCD_FillRect(uint32_t Xpos, uint32_t Ypos, uint32_t Width, uint32_t Height, uint32_t Color);
	  UTIL_LCD_FillRect(x_size/2-50, y_size/2 - 20, 100,40, UTIL_LCD_COLOR_WHITE);
	  ...
	  UTIL_LCD_DisplayStringAt(x_size/2-50, y_size/2, (uint8_t *)"Display freq.", LEFT_MODE);
...
/*		  MX_GPIO_Init();
	  ...
	  AudioFreq_ptr = &AudioFreq[5]; /* AUDIO_FREQUENCY_16K; */
	  ...
	  while (1)
	  {
		  ...
		  if (num_press == 0)
		  {
			  if (TS_State.TouchDetected>0&& touch==0)
			  {
				  ...
				  if (TS_State.TouchX>x_size/2-40 && TS_State.TouchX<x_size/2+40 && TS_State.TouchY>y_size/2-20 && TS_State.TouchY<y_size/2+20)
				  {
					  UTIL_LCD_Clear(UTIL_LCD_COLOR_BLACK);
					  num_press++;
				  }
	  		...
	  		HAL_SAI_Receive(&haudio_in_sai, (uint8_t *) &recordPDMBuf, AUDIO_IN_PDM_BUFFER_SIZE, 10);

			//SCB_InvalidateDCache_by_Addr((uint32_t *)&recordPDMBuf[AUDIO_IN_PDM_BUFFER_SIZE/2], AUDIO_IN_PDM_BUFFER_SIZE*2);

			BSP_AUDIO_IN_PDMToPCM(1, (uint16_t*)&recordPDMBuf[0], (uint16_t *) &RecPlayback[playbackPtr]);

			// Clean Data Cache to update the content of the SRAM
			//SCB_CleanDCache_by_Addr((uint32_t*)&RecPlayback[playbackPtr], AUDIO_IN_PDM_BUFFER_SIZE/4);
			playbackPtr += AUDIO_IN_PDM_BUFFER_SIZE/sizeof(uint16_t)/4/2;
			if(playbackPtr >= AUDIO_BUFF_SIZE)
			{
				playbackPtr = 0;
				for (int i = 0; i < FFT_LENGTH; i++)
				{
					input_fft[i] = ((float32_t)((int16_t) RecPlayback[i*2]))/65536.0f;//*window[i];
				}
				...
				//arm_max_f32(&output_fft_mag[1], FFT_LENGTH /2 - 512, &maxValue, &maxIndex);
				if (num_press > 0)
				{
					UTIL_LCD_Clear(UTIL_LCD_COLOR_BLACK);
					for (int i = 2; i < x_size; i++) // Start from the second element
					{
						UTIL_LCD_FillRect(i-2, 1, 1, (int) (output_fft_mag[i] * (y_size/30)), UTIL_LCD_COLOR_WHITE);
...
/*  defaultTaskHandle = osThreadNew(StartDefaultTask, NULL, &defaultTask_attributes);
  micAcqTaskHandle = osThreadNew(StartMicTask, NULL, &micTask_attributes);
...
void StartMicTask(void *argument)
{
  ...
  for(;;)
  {
		...
		if (playbackPtr == 0)
		{
		  ...
		  UTIL_LCD_FillRect(400/2-50, 200/2 - 20, 100,40, UTIL_LCD_COLOR_BLACK);
...
void StartFftTask(void *argument)
{
  ...
  for(;;)
  {
		...
		uint32_t    maxIndex;
		//arm_max_f32(&output_fft_mag[1], FFT_LENGTH /2 - 512, &maxValue, &maxIndex);
	    for (int i = 2; i < FFT_LENGTH /2 - 512; i++) { // Start from the second element
	        if (output_fft_mag[i] > maxValue) {
	            maxValue = output_fft_mag[i]; // Update maxValue if current element is larger
	            maxIndex = i;
	        }
	    }

	    x = maxIndex * 44100/2048;
  }
  /* USER CODE END 5 */
}
