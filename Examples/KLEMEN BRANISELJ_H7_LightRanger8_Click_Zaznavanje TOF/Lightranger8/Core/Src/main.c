...
#include "lightranger8.h" // Make sure to include your HAL header
...
#define LIGHTRANGER8_MODEL_ID_REG 0x010F
#define LIGHTRANGER8_EXPECTED_MODEL_ID 0xEA


void lightranger8_power_on(lightranger8_t *ctx);


void display_sensor_status(lightranger8_t *ctx) {
    char SendBuffer[BUFSIZE];
    uint8_t model_id = lightranger8_read_reg(ctx, LIGHTRANGER8_MODEL_ID_REG);

    // Format the message with the register value
    snprintf(SendBuffer, BUFSIZE, "Sensor Model ID Register Value: 0x%02X\n\r", model_id);
...
extern I2C_HandleTypeDef hi2c4; // Replace with your actual I2C handle

// Function to read distance from the sensor
// Usage in your distance reading function


// Function to read a 16-bit value from the sensor register
// Source file definition without static
...
/* Configure the peripherals common clocks */
  ...
  uint8_t read_data_u8 (lightranger8_t *ctx,uint16_t MEMORY_ADDRESS )
    ...
    uint16_t read_data_u16 ( lightranger8_t *ctx,uint16_t MEMORY_ADDRESS )
    {
    	uint8_t bytes[2];
    ...
    uint32_t read_data_u32 (lightranger8_t *ctx, uint16_t MEMORY_ADDRESS )
    {
    	uint8_t bytes[4];

    	retval = HAL_I2C_Mem_Read(&hi2c4,ctx->i2c_address,MEMORY_ADDRESS,I2C_MEMADD_SIZE_16BIT,bytes,4,HAL_MAX_DELAY
    	);

    	return (uint32_t)bytes[0] << 24 | (uint32_t)bytes[1] << 16 | (uint32_t)bytes[2] << 8 | bytes[3];
    }

    void write_data_u8 (lightranger8_t *ctx, uint16_t MEMORY_ADDRESS, uint8_t data )
    {
    ...
    void write_data_u16 ( lightranger8_t *ctx,uint16_t MEMORY_ADDRESS, uint16_t data )
    {
    	uint8_t bytes[2];
    	bytes[0] = data >> 8;
    	bytes[1] = data & 0x00FF;

    	retval = HAL_I2C_Mem_Write(&hi2c4,ctx->i2c_address,MEMORY_ADDRESS,I2C_MEMADD_SIZE_16BIT,bytes,2,HAL_MAX_DELAY);
    }


    void write_data_u32 (lightranger8_t *ctx, uint16_t MEMORY_ADDRESS, uint32_t data )
    {
    	uint8_t bytes[4];
    	bytes[0] = data >> 24;
  ...
  */
  ...
  void lightranger8_set_xshut_pin(lightranger8_t *ctx, GPIO_PinState state) {
      HAL_GPIO_WritePin(ctx->en_port, ctx->en_pin, state);  // HAL GPIO write function
  }

  void lightranger8_power_on(lightranger8_t *ctx) {
      // Set XSHUT pin low to power down the sensor
      lightranger8_set_xshut_pin(ctx, 0);  //  GPIO_PIN_RESET 0

      // Delay to allow the sensor to power up
      HAL_Delay(100);

      // Set XSHUT pin high to power on the sensor
      lightranger8_set_xshut_pin(ctx, 1);    // GPIO_PIN_SET  1

      // Delay to allow the sensor to fully power on
      HAL_Delay(1000);
  }

  uint8_t lightranger8_get_interrupt_state(lightranger8_t *ctx) {
      return (uint8_t)HAL_GPIO_ReadPin(ctx->int_pin_port, ctx->int_pin);
  }
  ...
  void lightranger8_gpio_init(lightranger8_t *ctx) {
      GPIO_InitTypeDef GPIO_InitStruct = {0};

      // Enable the GPIO clock for the EN (Xshut) pin
      if (ctx->en_port == GPIOA) __HAL_RCC_GPIOA_CLK_ENABLE();
      else if (ctx->en_port == GPIOB) __HAL_RCC_GPIOB_CLK_ENABLE();
      // Add other ports as needed...

      // Configure EN (Xshut) pin as output
      GPIO_InitStruct.Pin = ctx->en_pin;
      GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
      GPIO_InitStruct.Pull = GPIO_NOPULL;
      GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
      HAL_GPIO_Init(ctx->en_port, &GPIO_InitStruct);

      // Enable the GPIO clock for the interrupt pin
      if (ctx->int_pin_port == GPIOA) __HAL_RCC_GPIOA_CLK_ENABLE();
      else if (ctx->int_pin_port == GPIOB) __HAL_RCC_GPIOB_CLK_ENABLE();
      // Add other ports as needed...

      // Configure interrupt pin as input
      GPIO_InitStruct.Pin = ctx->int_pin;
      GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
      GPIO_InitStruct.Pull = GPIO_NOPULL;  //
      HAL_GPIO_Init(ctx->int_pin_port, &GPIO_InitStruct);
  }


  void lightranger8_en_high(lightranger8_t *ctx) {
      HAL_GPIO_WritePin(ctx->en_port, ctx->en_pin, 1);
  }

  void lightranger8_en_low(lightranger8_t *ctx) {
      HAL_GPIO_WritePin(ctx->en_port, ctx->en_pin, 0);
  }


  uint32_t calc_macro_period ( lightranger8_t *ctx, uint8_t vcsel_period ) {
  ...
  void lightranger8_set_measurement_timing_budget ( lightranger8_t *ctx, uint32_t budget_us ) {
  ...
  void delay_data_ready ( lightranger8_t *ctx ) {
  	HAL_Delay(1);
  }
  ...
  void lightranger8_calibrate_offset ( lightranger8_t *ctx, uint16_t target_distance_mm, uint32_t period_ms, int16_t *offset ) {
      ...
      for (i = 0; i < averaging_number; i++) {
          while ( lightranger8_get_interrupt_state( ctx ) != 0 ) {
...
/*
  ...
  void lightranger8_init(lightranger8_t *ctx, lightranger8_cfg_t *cfg) {
        // Initialize I2C
	  /*
        ctx->i2c_handle->Instance = I2C4;
        ctx->i2c_handle->Init.Timing = cfg->i2c_speed;  // I2C speed
        ctx->i2c_handle->Init.OwnAddress1 = 0;
        ctx->i2c_handle->Init.AddressingMode = I2C_ADDRESSINGMODE_7BIT;
        ctx->i2c_handle->Init.DualAddressMode = I2C_DUALADDRESS_DISABLE;
        ctx->i2c_handle->Init.OwnAddress2 = 0;           // Set to 0 if not used
  ...
  {
      uint8_t tmp = read_data_u8(ctx, LIGHTRANGER8_GPIO_TIO_HV_STATUS );
...
/*
			...
			snprintf(SendBuffer,BUFSIZE,"For calibration place an object at %.1f mm distance from sensor.\r\n", ( float )calibration_distance_mm / 10 );
			HAL_UART_Transmit(&huart3,SendBuffer,strlen(SendBuffer),100);
			HAL_Delay(1000);
			HAL_Delay(1000);
			HAL_Delay(1000);
			HAL_Delay(1000);
			HAL_Delay(1000);
			snprintf(SendBuffer,BUFSIZE," ---------------    Sensor calibration is in progress...     ---------------\r\n");
			HAL_UART_Transmit(&huart3,SendBuffer,strlen(SendBuffer),100);
			...
			HAL_Delay(500);
			snprintf(SendBuffer,BUFSIZE," -------------    Sensor measurement commencing...    -------------\r\n");
			HAL_UART_Transmit(&huart3,SendBuffer,strlen(SendBuffer),100);
...
/*
	 	  ...
	 	  // uint8_t dataBuffer;  // Buffer to hold the read data
	 	  // HAL_StatusTypeDef retval = HAL_I2C_Mem_Read(&hi2c4, (0x29 << 1), 0x0110, I2C_MEMADD_SIZE_16BIT, dataBuffer, 1, HAL_MAX_DELAY);

	 	   /*
	 	   snprintf(SendBuffer, BUFSIZE, "LIGHTRANGER ID: 0x%02X %s\n\r",
	 	            dataBuffer[0], (retval == HAL_OK) ? "" : "I2C Read Error");
...
/*
	 	    ...
	 	    retval = HAL_I2C_Mem_Read(&hi2c4, (0x29 << 1), 0x010F, I2C_MEMADD_SIZE_16BIT,dataBuffer, 1, HAL_MAX_DELAY);


	 	    snprintf(SendBuffer,BUFSIZE,"Hello World [%d]: Key:%d Audio Chip ID: 0x%4x LIGHTRANGER ID: 0x%2x=%d\n\r",Counter++,KeyState, dataBuffer[0]*256+dataBuffer[1],dataBuffer[5],dataBuffer);
	 	    HAL_UART_Transmit(&huart3,SendBuffer,strlen(SendBuffer),100);
